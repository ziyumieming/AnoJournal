...
class AdvancedParticleSystem {
  ...
  private initEventListeners(): void {
    ...
    this.areaElement.addEventListener('dblclick', this.handleDoubleClick.bind(this));
    this.areaElement.addEventListener('mousedown', this.handleMouseDownForLongPress.bind(this));
    ...
    if (this.isMouseStationary) { // 如果之前是静止的，现在开始移动了
      ...
      this.particles.forEach(p => {
        ...
        if (p.state === ParticleState.ORBITAL || p.state === ParticleState.DISPERSING_TO_ORBIT) {
           if (p.state !== ParticleState.EVENT_SHRINKING && p.state !== ParticleState.EXPLODING) { // Don't override active event states
...
private handleClickOrDoubleClick(event: MouseEvent, isDoubleClick: boolean): void {
    ...
    if (this.longPressInfo.isActive || this.longPressInfo.isDetecting) {
        ...
        if (!isDoubleClick && !this.longPressInfo.isActive) { // If just detecting, a quick click might cancel it
...
private handleMouseDownForLongPress(event: MouseEvent): void {
    ...
    this.longPressInfo.detectorTimerId = setTimeout(() => {
        if (this.longPressInfo.isDetecting) { // Still valid (mouse hasn't moved away or up)
    ...
    window.addEventListener('mouseup', this.boundHandleMouseUpForLongPress, { once: true }); // Listen globally, once
...
private handleMouseUpForLongPress(event: MouseEvent): void {
    ...
    if (this.longPressInfo.isActive) { // A long press was successfully completed
        ...
        const explosionSpeed = this.EXPLODE_BASE_SPEED + 
            (Math.floor(this.longPressInfo.duration / 100) * this.EXPLODE_SPEED_BOOST_PER_100MS);
        ...
        const numToActuallyExplode = Math.max(particlesToExplode.length, Math.floor(this.NUM_ORBITAL_PARTICLES * 0.5));


        for (let i = 0; i < numToActuallyExplode; i++) {
            const p = (i < particlesToExplode.length) ? particlesToExplode[i] : { // Reuse or create new
        ...
        this.mouse.lastMoveTime = Date.now() - (this.STATIONARY_THRESHOLD_MS + 10);
...
private handleMouseMoveForLongPress(event: MouseEvent): void {
    if (this.longPressInfo.isDetecting && !this.longPressInfo.isActive) {
        const dx = event.clientX - (this.areaElement.getBoundingClientRect().left + this.longPressInfo.startX);
        const dy = event.clientY - (this.areaElement.getBoundingClientRect().top + this.longPressInfo.startY);
        if (Math.sqrt(dx * dx + dy * dy) > this.LONG_PRESS_MAX_MOVE_THRESHOLD_PX) {
            this.cancelLongPress();
        }
    }
}
...
private cancelLongPressState(): void {
    ...
    window.removeEventListener('mousemove', this.boundHandleMouseMoveForLongPress);
}
...
private updateSystemState(): void {
    ...
    if (!this.longPressInfo.isActive && !this.isMouseStationary && (now - this.mouse.lastMoveTime > this.STATIONARY_THRESHOLD_MS)) {
      ...
      for (let i = 0; i < candidatesForDispersal.length && dispersedCount < this.NUM_ORBITAL_PARTICLES; i++) {
        ...
        if (p.state !== ParticleState.EVENT_SHRINKING && p.state !== ParticleState.EXPLODING) { // Check again
            p.state = ParticleState.DISPERSING_TO_ORBIT;
            ...
            p.orbitalSpeed = this.ORBITAL_BASE_SPEED + (Math.random() - 0.5) * this.ORBITAL_SPEED_VARIATION; 
            dispersedCount++;
        }
      }
  ...
  private updateParticles(): void {
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      if (p.state !== ParticleState.ORBITAL || p.life !== Infinity) {
          if (p.state !== ParticleState.EXPLODING || p.life !== Infinity) { // Exploding also has life
             p.life--;
          }
      }
      ...
      switch (p.state) {
        ...
        case ParticleState.SHRINKING: // 粒子追赶鼠标
          ...
          break;
            ...
            if (p.eventShrinkTargetX === undefined || p.eventShrinkTargetY === undefined) {
                this.particles.splice(i, 1); continue; // Should have a target
            }
            const dx_event_shrink = p.eventShrinkTargetX - p.x;
            const dy_event_shrink = p.eventShrinkTargetY - p.y;
            const dist_event_shrink = Math.hypot(dx_event_shrink, dy_event_shrink);
            ...
            if (dist_event_shrink < this.SHRINK_SNAP_DISTANCE || p.life <= 0) {
                ...
                if (!this.longPressInfo.isActive || p.life <= 0) { // If not an active long press or life ended
                    this.particles.splice(i, 1);
                } else { // For active long press, hold at center
        ...
        case ParticleState.EXPLODING:
            ...
            if (p.opacity <= 0) { // Also check opacity if life isn't perfectly synced
                 this.particles.splice(i, 1);
                 continue;
            }
            break;
